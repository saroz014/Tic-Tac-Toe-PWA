...
function minimax(state, depth, player) {
  ...
  if (depth == 0 || gameOverAll(state, COMP, HUMAN)) {
...
function clickedCellAI(cell) {
  ...
  var conditionToContinue =
    gameOverAll(board, COMP, HUMAN) == false && emptyCells(board).length > 0;
  ...
  if (emptyCells(board).length == 0 && !gameOverAll(board, COMP, HUMAN)) {
    var msg = document.getElementById("message");
    msg.innerHTML = "Draw!";
  }
  if (
    gameOverAll(board, COMP, HUMAN) == true ||
